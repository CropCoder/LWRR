mod_population_UI <- function(id) {
  ...
  tagList(
      ...
      tags$div(class = "alert alert-success", role = "alert",
               strong("Tip: "), "The Population page provides geographic source and structural group information for all accessions. On this page, you can view the structural characteristics of different subgroups and obtain the sample information differences of wheat breeding groups(BG1-BG4), landrace(LC) and non-landrace(NLC)."),
      # word map ----
      h3("Population source world map"),

      card(
          tags$img(src="fig/population_wordmap.png", style = "width: 100%;"),
          p(" ✨ Annotation: The samples come from all over the world and have a wide range of regional and genetic diversity. 272 from Africa, 929 from Asia, 640 from Europe, 298 from Americas, 52 from Oceania. This panel consisted of 684 landraces and 1,507 cultivars.")
      ),
      ...
      card(
          ...
          p("The above results are based on the Admixture（https://dalexander.github.io/admixture） software for population structure analysis of genotypes of 2191 materials， and presented the data of 2191 materials in each component uniformly. You can click on any material to obtain the corresponding calculation result, and the local selection function is provided below the X-axis, making it convenient for you to view a specific location.")
      ),
      
      # PCA ----
      h3("Principal Component Analysis of Population"),
      tags$div(class = "alert alert-success", role = "alert",
               strong("Tip："), "We provide the PCA analysis results of 2191 materials at the population level. The left image below represents the PCA results of Breeding Group and landrace, while the right image represents the population composition coloring results calculated based on structure. You can drag and drop with the mouse to view, or click on the legend to interactively view the distribution of a specific subgroup."),
      layout_column_wrap(
          1/2,
          card(
              card_header("PCA 3D Plot (color by Admixture k=9 )"),
              withSpinner(plotlyOutput(ns("PCA_3D_K9"))),full_screen = T
          ),
          card(
              card_header("PCA 3D Plot (color by Breed Group)"),
              withSpinner(plotlyOutput(ns("PCA_3D_BG"))),full_screen = T
          )
      ),

      # breedgroup ----
      h3("Query the information and characteristics of wheat breeding group (BG)"),
      radioGroupButtons(
      ...
      card(
          ...
          withSpinner(DTOutput(ns("table_BG"))),
          hr(),
          br(),
          layout_column_wrap(
              1/3,
          ...
          tags$div(class = "alert alert-success", role = "alert",
                   strong("Note："), "Breeding group（BG） are divided according to multiple factors such as wheat breeding history, distribution of main planting areas, population structure characteristics, geographical origin and kinship, and are used to distinguish the four major wheat breeding groups in the world."),
      ),
      
      
      HTML("<h3>Diversity and selection of different breeding groups (Pi & <i>F</i><sub>ST</sub>)</h3>"),
      # radioGroupButtons(
      ...
      fluidRow(
          ...
          column(
              8,
              p("Tips:"),
              tags$div(class = "alert alert-danger", role = "alert","Note：Please check that the range size cannot exceed 100Mb. Otherwise, the system may crash."),
              p("1. Pi was calculated using samples from BG1-BG4, landrace（LC）, and non-landrace（NLC）"),
              HTML("<p>2. <i>F</i><sub>ST</sub> was calculated using the resistant samples in each breeding group compared with the landrace</p>"),
              p("3. The sliding window step was 0.1Mb and the window size was 100kb ( vcftools , https://vcftools.github.io/)")
          )
      ),
      card(
                      full_screen = T,
                      echarts4rOutput(ns("pop_pi_plot"),width = "100%",height = "400px"),
                      br("Nucleotide diversity (π) of each breeding group can reflect the polymorphism of variation sites in different physical regions. This allows researchers to quantify the genetic diversity within a population, which reflects the average percentage of base differences between individual DNA sequences within the population")
                  ),
      card(
                      full_screen = T,
                      echarts4rOutput(ns("pop_fst_plot"),height = "500px"),
                      HTML("<p><i>F</i><sub>ST</sub> is a measure of population differentiation and genetic distance. The greater the differentiation index, the greater the difference between subpopulations. It can be used to compare the diversity of subgroups and measure the degree of population differentiation.</p>")
                  )
    ...
    #                   echarts4rOutput(ns("pop_fst_plot"),height = "500px"),
    #                   br(),
    #                   p("Fixation index (Fst) is a measure of population differentiation and genetic distance. The greater the differentiation index, the greater the difference between subpopulations. It can be used to compare the diversity of subgroups and measure the degree of population differentiation.")
    #               )
    #           )
    #       ),
        ...
        #               br(),
        #               p("Tajmd is used to check whether sequences follow a neutral evolution model during evolution.")
        #           )
        #       ),
        #       list(
        #           # snpnum----
        ...
        #               br(),
        #               p("The distribution of variation sites on chromosomes is different.")
        #           )
        #       )
...
mod_population_Server <- function(id) {
  ...
  "))
        ...
        # table_BG----
        output$table_BG <- renderDT({
            select_BG <- input$select_which_BG
            data <- S1_sample[which(S1_sample$BreedingGroup == select_BG),
                              c("RunID","Name","Status","Year","SubRegion")]
            
            # 设置列名
            colnames(data) <- c("SampleID","Name","Growth habit","Breeding time","SubRegion")
            
            data$Link <- paste0('<a href="',"./?page=Sample&Sample_ID=",data$SampleID, '";target="_blank";style="text-decoration: none;">Click Here to view the detail</a>')
        ...
        output$BG_Year_info <- renderEcharts4r({
            ...
            data_summary$Year <- data_summary$Year %>% str_replace("Y","")
  ...
  ")) %>%
                e_title(str_c("Breeding Year of ",input$select_which_BG," Samples"),x="center") %>% 
                e_legend(show = F)
        ...
        output$BG_K_info <- renderEcharts4r({
            ...
            data_summary$K9 <- paste0("Subgroup ",data_summary$K9 %>% str_remove("K"))
  ...
  ")) %>%
                e_title(str_c("Structure Subgroup of ",input$select_which_BG," (K=9)"),x="center") %>% 
                e_legend(show = F)
  ...
  ")) %>%
                e_title(str_c("Geographical distribution of ",input$select_which_BG),x="center") %>% 
                e_legend(show = F)
