...
mod_home_ui <- function(id, label = "home") {
  ...
  tagList(
    ...
    fluidRow(
      column(
        ...
        shinycssloaders::withSpinner(
          slickROutput(ns("top_fig"), height = "300px", width = "103%"),
          type = 8, color = "#a4b0be"
        )
      ),
      column(
        ...
        card(
          ...
          tags$img(src = "fig/new_home_tech.png", style = "width: 100%;")
        )
      )
    ),
    tags$div(
      class = "alert alert-success", role = "alert",
      includeMarkdown("www/md/home_down.md")
    ),
    # # 提示信息卡片
    #   tags$div(
    #     style = "
    ...
    #       ",
    #     icon("info-circle"),
    #     "If use LWRR in pulication, please cite >>>>>> LWRR: A database of Landscape of Wheat Rust Resistance promoting design breeding of disease resistance (DOI....)"
    #   ),
    br()
    # trait----
    # h3("Trait Overview"),
            ...
            if (myinput %in% search_index_EN_name) {
              ...
              if (myinput %in% search_index_QYr) {
                ...
                if (myinput %in% search_index_GeneName) {
                  ...
                  if (myinput %in% search_index_GeneID) {
                    shinyjs::runjs(paste0("window.location.href = '", "./?page=Gene&Gene_ID=", myinput, "';"))
                  } else {
