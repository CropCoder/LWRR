mod_Gene_UI <- function(id) {
  ...
  tagList(
      ...
      tags$div(class = "alert alert-success", role = "alert",
               strong("Note："), "You can search for a particular QTL of interest on this page and get the corresponding information,the landscape contains a total of 431 QTLs. This page only shows the information of candidate disease resistance genes located in the QTL."),
      ...
      layout_column_wrap(
          ...
          card(
              textInputIcon(
                  ...
                  value = "TraesCS2A03G1330400",
                  placeholder = "TraesCS2A03G1330400",
      ...
      # which QTL----
      h3("Gene Expression near QTL" ),
      
      card(full_screen = T,
          reactableOutput(ns("QTL_genes_TPM")),
      ...
      card(
          ...
          layout_column_wrap(
              ...
              list(
                  ...
                  card(
                      full_screen = T,
                      echarts4rOutput(ns("pop_pi_plot"),width = "100%",height = "600px")
              ...
              list(
                  ...
                  card(
                      full_screen = T,
                      echarts4rOutput(ns("pop_fst_plot"),width = "100%",height = "600px")
      ...
      h3("Variation sites in gene regions ( WGS + 660K )"),
      card(
          card_header("Jbrowse (This page is supported based on WheatOmics)"),
          uiOutput(ns("wheatomics_vcf")),full_screen = T
      )
...
mod_Gene_Server <- function(id) {
  moduleServer(
    ...
    function(input, output, session) {
        ...
        # 创建响应值- 默认
        react_vals <- reactiveValues(Gene = "TraesCS2A03G1330400")
        ...
        observe({
            ...
            output$gene_desc <- renderText(
                paste0("👀 Description: ",Gene_Search_out$gene_data[1,7])
            )
                    ...
                    tagList(
                        tags$div(
                            ...
                            tags$div(
                                ...
                                tags$div(style = bar_style, class = "progress-bar-inner",  # 浅蓝色进度条部分，添加类名方便后续进一步样式调整（可选）
                                         role = "progressbar",
                                         `aria-valuenow` = value,
                                         `aria-valuemin` = 0,
  ...
  "))
            ...
            output$wheatomics_vcf <- renderUI({
                list(
                    HTML(str_c("<iframe src='http://202.194.139.32/jbrowse-1.12.3-release/?",
            ...
            # jbrowse----
            # output$Gene_jb <- renderJBrowseR({
            #     # assembly
            #     assembly <- assembly(
            #         "https://wheatomics.oss-cn-chengdu.aliyuncs.com/LWDR/WheatCS21.fa.gz",
            #         bgzip = TRUE
            #     )
            #     
            #     # create configuration for a JB2 GFF FeatureTrack
            #     # annotations_track <- track_feature(
            #     #     "https://wheatomics.oss-cn-chengdu.aliyuncs.com/LWDR/WheatCS21.gff3.gz",
            #     #     assembly
            #     # )
            #     
            #     annotations_track <- "{ \"type\": \"FeatureTrack\", \"name\": \"WheatCS21\", \"assemblyNames\": [\"WheatCS21\"], \"trackId\": \"WheatCS21_WheatCS21\", \"adapter\": { \"type\": \"Gff3TabixAdapter\", \"gffGzLocation\": { \"uri\": \"https://wheatomics.oss-cn-chengdu.aliyuncs.com/LWDR/WheatCS21.gff3.gz\" }, \"index\": { \"location\": { \"uri\": \"https://wheatomics.oss-cn-chengdu.aliyuncs.com/LWDR/WheatCS21.gff3.gz.csi\" } , \"indexType\": \"CSI\"} }}"
            #     
            #     
            #     # vcf_track <- track_variant(
            #     #     "https://wheatomics.oss-cn-chengdu.aliyuncs.com/LWDR/20240227_QTL_SNP.vcf.recode.vcf.gz",
            #     #     assembly
            #     # )
            #     
            #     # vcf_track <- "{ \"type\": \"VariantTrack\", \"name\": \"20240227_QTL_SNP\", \"assemblyNames\": [\"WheatCS21\"], \"trackId\": \"WheatCS21_20240227_QTL_SNP\", \"adapter\": { \"type\": \"VcfTabixAdapter\", \"vcfGzLocation\": { \"uri\": \"http://202.194.139.32/jbrowse-1.12.3-release/Chinese_Spring2.1/tracks/vcf/wgs.1618.wheat660k.623.merge.maf0.01.2191.ann.vcf.gz\" }, \"index\": { \"location\": { \"uri\": \"http://202.194.139.32/jbrowse-1.12.3-release/Chinese_Spring2.1/tracks/vcf/wgs.1618.wheat660k.623.merge.maf0.01.2191.ann.vcf.gz.tbi\"  } , \"indexType\": \"tbi\"} }}"
            #     
            #     # create the tracks array to pass to browser
            #     tracks <- JBrowseR::tracks(
            #         annotations_track
            #         # vcf_track
            #     )
            #     
            ...
            #     )
            #     
            #     theme <- theme("#5da8a3", "#4b704b")
            #     
            #     JBrowseR(width = "100%",height = "500px",
            #         "View",
            #         assembly = assembly,
            #         tracks = tracks,
            #         # 动态获取位置参数
            #         location = str_c(
            #             "Chr",Gene_Search_out$chr,":",Gene_Search_out$start-1000,"-",Gene_Search_out$end+1000
            #         ),
